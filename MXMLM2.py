...
if os.path.isfile("correspondence.xml"):
    ...
    for eachID in documentIDs:
        ...
        # I checked the outputs - they were semantically identical with and without it.
        # Do not remove it.
        ...
        CorrespDict[eachID]['recipientsRef'] = ["https://www.wikidata.org/wiki/Q41406"]
    ...
    print("Complete - wrote to correspondence.json")
else:
...
if os.path.isfile("register_tei.xml"):
    ...
    for eachID in documentIDs:
        ...
        RegDict[eachID]['authorsRef'] = ["https://www.wikidata.org/wiki/Q41406"]
    ...
    with open("registry.json", "w") as outfile:
        outfile.write(json_object)
    print("Complete - wrote to registry.json")
else:
    print("No correspondence.xml file provided. MXML will not munch letters to Munch.")
...
print("Wrote MXMLM_Output_Combined.json (unified dataset including optional files)")
...
print("Saving output...")
with open("CMIF_Output_Default.xml", "w", encoding="utf-8") as outfile:
    outfile.write(CMIF.prettify())
with open("CMIF_Output_NoSortAttrs.xml", "w", encoding="utf-8") as outfile:
    outfile.write(CMIF.prettify(formatter=UnsortedAttributes()))
print("OK\nSaved CMIF_Output_Default.xml (attributes in alphabetical order)\nSaved CMIF_Output_NoSortAttrs.xml (attributes in logical order)")

#x = CMIF.find("correspDesc")
